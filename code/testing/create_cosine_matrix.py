...
import time
from code.testing import cosine_similarity
...
def similar_for_id(film_ids, database, how_many, que):
    for film in film_ids:
        similars = cosine_similarity.find_most_similar(film, database, how_many)
        similars["movie"] = film
        que.put(similars)
    que.put(None)
    # print(similars)
    ...
    while (workers > 0):
        if not que.empty():
            ...
            if foo is None:
                ...
                foo.to_csv(target, index=False, header=first, mode="a")
                first = False
...
if __name__ == '__main__':
    ...
    # sample = database.head(FILMS_PER_WORKER * workers)
